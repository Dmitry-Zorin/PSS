		...
		async (req, res, next) => {
			try {
				...
				const {
					platoonNumber,
					companyNumber,
				} = await Model.findById(req.params.id).exec()
				...
				for (const employee of employees) {
					...
					const employeeTotalScore = redmineInfo.reduce((total, info) => total
						+ info.score, 0)
					...
					if (!response.scores.length) {
						...
						if (diff > 0) {
							response.scores.unshift(...scores.slice(0, diff)
								.map(e => e.score = 0))
						}
					...
					for (const [trackerName, value] of Object.entries(lastWeekInfo.hours)) {
						...
						response.hours[trackerName] =
							(response.hours[trackerName] || 0) + value
					}
				}
